...
export const Input = React.forwardRef(
  ({ label, type = 'text', error, icon: Icon, className = '', id, ...props }, ref) => {
    ...
    return (
      <div className={`flex flex-col w-full gap-1.5 ${className}`}>
        {label && (
          <label
            htmlFor={inputId}
            className="text-xs font-semibold text-app-text-secondary uppercase tracking-wider"
          >
            {label}
          </label>
        )}
        ...
        {error && (
          <span className="text-xs text-danger-500 font-medium">{error.message || error}</span>
        )}
      </div>
    );
  }
