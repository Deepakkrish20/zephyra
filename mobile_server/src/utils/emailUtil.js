...
const getTransporter = () => {
  ...
  if (!host || !user || !pass) {
    return null; // Fallback to console simulation
  }
...
export const sendVerificationEmail = async (email, code) => {
  ...
  const mailOptions = {
    from: `"Zephyra Mobile Support" <${process.env.SMTP_USER || 'no-reply@zephyra.io'}>`,
    to: email,
    subject: 'Verify Your Zephyra Mobile Account',
    text: `Your Zephyra verification code is: ${code}. This code is valid for 15 minutes.`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;">
        <h2 style="color: #7c3aed; text-align: center;">Welcome to Zephyra Mobile!</h2>
  ...
  if (!transporter) {
    console.log(`\n========================================`);
    console.log(`[Mobile Dev Fallback] Verification Email Simulation`);
